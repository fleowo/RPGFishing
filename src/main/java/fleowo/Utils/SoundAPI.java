...
public enum SoundAPI {
    ...
    private static final HashMap<String, SoundAPI> cachedSearch;

    SoundAPI(String... versionDependentNames) {
    ...
    public Sound bukkitSound() {
        ...
        for (i = (arrayOfString = this.versionDependentNames).length, b = 0; b < i; ) {
    ...
    public static SoundAPI requestXSound(String name) {
        if (cachedSearch.containsKey(name.toUpperCase()))
            return cachedSearch.get(name.toUpperCase());
        byte b;
        int i;
        SoundAPI[] arrayOfXSound;
        for (i = (arrayOfXSound = values()).length, b = 0; b < i; ) {
            SoundAPI snd = arrayOfXSound[b];
            byte b1;
            int j;
            String[] arrayOfString;
            for (j = (arrayOfString = snd.versionDependentNames).length, b1 = 0; b1 < j; ) {
